	...
	public void sendMail() {
		try {
			...
			String	bodyColaborador="<html>\r\n"
						...
						+ "</html>";
			MailDTO mail2 = new MailDTO(from,  
					Arrays.asList(c.getEmailEvaluated().toLowerCase()),
					new ArrayList<>(), 
					new ArrayList<>(), 
					subject, 
					bodyColaborador, 
					new ArrayList<>());
				System.out.println("Entrando a "+c.getEmailEvaluated().toLowerCase());
				sharedService.sendMail(Arrays.asList(mail2));
				System.out.println("Correo enviado a "+c.getEmailEvaluated().toLowerCase());
				c.setFinish(true);
				c.setEnter(true);
				c.setStatus(2);
				evaluatedRepo.save(c);
